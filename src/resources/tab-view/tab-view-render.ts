...
export class TabViewRender extends ViewModel {
  ...
  bind() {
    this.subscriptions.subd(d => {
      d(this.observeEx(x => x.selectedTab).filter(x => x != null).subscribe(x => this.lastActiveTab = x));
      d(this.observeEx(x => x.selectedTab).filter(x => x == null).delay(1000).map(x => this.selectedTab == null).subscribe(x => this.lastActiveTab = null));
