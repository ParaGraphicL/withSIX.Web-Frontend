...
export module Components.LoadingStatusInterceptor {
  ...
  export class LoadingStatusInterceptor extends Tk.Service {
    ...
    public requestError = (rejection) => {
      if (rejection.config && rejection.config.w6Request && Tools.debug) Tools.Debug.log("requestError", rejection);
    ...
    public response = (response) => {
      if (response.config && response.config.w6Request) {
        ...
        Tools.Debug.log("w6Response", response);
      } else if (response.config.breezeRequest) {
        Tools.Debug.log("breezeResponse", response);
      }
      this.ended();
    ...
    public responseError = (rejection) => {
      if (rejection.config) {
        if (rejection.config.w6Request) {
          ...
          Tools.Debug.log("w6ResponseError", rejection);
          ...
          Tools.Debug.log("breezeResponseError", rejection);
        }
      }
      this.ended();
      return this.$q.reject(rejection);
