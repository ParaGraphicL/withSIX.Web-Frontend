          ...
          return {
            ...
            link: ($scope: EmailScope, $element, $attrs: any, ctrl) => {
              ...
              if ($attrs.checkAlreadyExists) {
                ...
                $scope.checkExists = () => $rootScope.request<boolean>(EmailExistsQuery, { email: ctrl.email.$viewValue })
                  .then(result => {
                    $scope.$apply(() => {
          ...
          return {
            ...
            link: ($scope, $element, $attrs: any, ctrl) => {
              ...
              if ($attrs.checkAlreadyExists) {
                // TODO: Only if not other validations failed?
                // using viewValue as workaround because model not set when already invalid last time
                $scope.checkExists = () => $rootScope.request<boolean>(UsernameExistsQuery, { userName: ctrl.userName.$viewValue })
                  .then(result => {
                    $scope.$apply(() => {
...
export class RegisterDialogController extends DialogControllerBase {
  ...
  constructor($scope, logger, $modalInstance, $q) {
    ...
    $scope.register = () => this.requestAndProcessResponse(RegisterCommand, { data: $scope.model });
  }
}
