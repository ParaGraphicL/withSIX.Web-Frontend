...
import { OpenRegisterDialogQuery, ForwardService } from './components';
import './play';
import './games';
import './mods';
import './missions';
...
import { execute } from './binder';
import { configure as configurePlay } from './play-config'
...
export class MainAppController extends BaseController {
  ...
  openRegisterDialog(evt?) {
    ...
    return this.$scope.request(OpenRegisterDialogQuery);
  }
...
export class LoadingFailedController extends Tk.Controller {
  ...
  constructor($scope: IBaseScope, logger, private forwardService: ForwardService, error) {
    super($scope);
