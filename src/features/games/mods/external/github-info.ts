import {ViewModel, GitHub, Query, DbQuery, W6Context, handlerFor, UiContext, Post, GithubUrl} from '../../../../framework';
...
export class GitHubInfo extends ViewModel {
  ...
  async activate(repo: string) {
    try {
      ...
      this.eventBus.publish(new UpdateInterestingLinks([new GithubUrl(`https://github.com/${repo}`)]));
    } catch (ex) { }
  }
}
