...
export class Playlist extends ViewModel {
  ...
  updateCollection(c: IPlaylistCollection, startVal = false) {
    ...
    if (c != null) {
      ...
      listObs.subscribe(x => { Tk.Debug.log("$$$ list val", x); });
      objObs.subscribe(x => { Tk.Debug.log("$$$ obj val", x); });
      obs.subscribe(x => { Tk.Debug.log("$$$ merged val", x); });
  ...
  gameChanged = async (info: GameChanged) => {
    ...
    Tk.Debug.log("$$$ ClientBar Game Changed: ", info, equal);
    if (equal) return;
