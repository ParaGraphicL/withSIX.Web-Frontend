...
export class DownloaderHeader extends ViewModel {
  ...
  handleActionInfo = (actionInfo: IActionTabState) => {
    Tk.Debug.log("$$$ received actionTabState", actionInfo);
    if (actionInfo == null) {
  ...
  //   Tk.Debug.log("$$$ received actionNote", evt);
  //   this.tab.progressInfo = {
  ...
  handleActionUpdate = (x: IActionTabStateUpdate) => {
    Tk.Debug.log("$$$ received updated actionTabState", x);
  ...
  gameChanged = async (info: GameChanged) => {
    Tk.Debug.log("$$$ DownloadHeader Game Changed: ", info);
    ...
    Tk.Debug.log("$$$ Download header Game Info", this.gameInfo);
  }
}
