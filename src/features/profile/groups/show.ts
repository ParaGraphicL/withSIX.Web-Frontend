...
export class Show extends ViewModel {
  ...
  configureRouter(config: RouterConfiguration, router: Router) {
    ...
    config.map([
      ...
      { route: 'mods', name: 'mods', moduleId: 'features/profile/groups/mods/index', nav: true, title: 'Mods', settings: { icon: "icon withSIX-icon-Nav-Collection" } },
      { route: 'servers', name: 'servers', moduleId: 'features/profile/groups/servers/index', nav: this.features.groups && this.features.servers, title: 'Servers', settings: { icon: "icon withSIX-icon-Nav-Server" } }
    ])
  }
}
